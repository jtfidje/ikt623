...
class AI:
	def __init__(self, piece, opponent, max_depth=3):
		# Game pieces. 
		self.ai = piece
...
"""
self.game.board = copy.deepcopy(self.node.board)
if self.game.put_piece(move, piece):
	node = Node(self.game.board, self.node, move, self.node.alpha, self.node.beta)

	# Evaluate game - check if leaf node is found
	if self.game.check_winner():
		self.move = move	# Save move. (May not be necessary)
		if self.node.parent == None:
			return True
		# If calc_heuristics return true, 
		# prune self.node by removing it
		# from self.node.parent.children list
		if self.calc_heuristics(node):
			if self.node in self.node.parent.children:
